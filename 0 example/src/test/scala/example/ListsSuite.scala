...
class ListsSuite extends FunSuite {
  ...
  test("one plus one is three?") {
    assert(1 + 1 != 3) // This assertion fails! Go ahead and fix it. DONE
  }
  ...
  test("details why one plus one is not three") {
    assert(1 + 2 === 3) // Fix me, please! DONE
  }
  ...
  // Does not compile: list elements are type-safe
  // type mismatch;  found   : String("one")  required: Int
//  test("sum of non-integer values") {
//    assert(sum(List("one", "two", "three")) === 6)
//  }
  ...
  test("sum of positives and negatives") {
    assert(sum(List(-1, 2, -3, 4, 0)) === 2)
  }
  
  test("sum of large magnitude") {
    assert(sum(List(Int.MinValue, Int.MaxValue)) === -1)
  }
  
  test("sum exceeds max pos") {
	  assert(sum(List(Int.MaxValue, 1)) === Int.MinValue) // specified by JVM
  }
  
  test("sum exceeds min pos") {
	  assert(sum(List(Int.MinValue, -1)) === Int.MaxValue) // specified by JVM
  }
  ...
  test("max of zero, postives and negative") {
